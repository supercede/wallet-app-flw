...
module.exports = {
  fundWallet: async (request, response) => {
    ...
    if (resp.body.data.suggested_auth) {
      ...
      // console.log('here resp 2 -->', reCallCharge.body);
  ...
  async validatePaymentManual({ request, response }) {
    const trx = await sequelize.transaction();

    const { flwRef, otp } = request.body;
    const cardTxn = await card_transactions.findBy(
      'external_reference',
      flwRef,
    );
    ...
    try {
      ...
      if (data.tx.status === 'failed') {
        ...
        console.log(cardTxn.failure_reason);
